...
<form action="<?php echo url_for('call/submit') ?>" method="POST" style="width: 100%;">
  <center>
    ...
    <input type="text" placeholder="<?php echo __('phone', null, 'call') ?>" class="ContactPhone" name="phone" id="phone"  style="width: 250px;
height: 30px;
border-radius: 3px;
...
color: #8E4702;" id="sub"/>
    ...
    <p><?php echo __('invite_message', null, 'call') ?></p>
  </center>
  <script>
	...
	$("#sub").click(
		function(){
			...
			{
				$("#err").text("<?php echo __('wrong', null, 'call') ?>");
				return false;
			}
		}
	);
...
<a href="<?php echo url_for('homepage') ?>" title="Арендовать катер, яхту или теплоход у судовладельца">Теплоходы, катера и яхты</a> - заказывайте аренду и организацию праздников у судоходной компании Рентфлот!
<?php endif; ?>