 ...
 */
class sfWidgetFormMagicNumberRange extends sfWidgetForm
{
  /**
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    $this->addOption('template', sfContext::getInstance()->getI18N()->__('from', array(), 'common').' %from_number% '.sfContext::getInstance()->getI18N()->__('to', array(), 'common').' %to_number%');
  ...
  public function renderTo($name, $value = null, $attributes = array(), $errors = array())
  {
    //$values = array_merge(array('from' => '', 'to' => '', 'is_empty' => ''), is_array($value) ? $value : array());   
    unset($attributes['type']);
    return $this->getOption('to_number')->render($name.'[to]', $value['to'], $attributes);
  }  

  public function renderFrom($name, $value = null, $attributes = array(), $errors = array())
  {
    //$values = array_merge(array('from' => '', 'to' => '', 'is_empty' => ''), is_array($value) ? $value : array());    
    unset($attributes['type']);
    return $this->getOption('from_number')->render($name.'[from]', $value['from'], $attributes);
  }    
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    if (!empty($attributes['type']) && $attributes['type'] == 'from' ) {
      return strtr($this->getOption('filter_template'), array(
        '%number_range%'     => $this->renderFrom($name, $value, $attributes, $errors),
    ...
    if (!empty($attributes['type']) && $attributes['type'] == 'to') {
      return strtr($this->getOption('filter_template'), array(
        '%number_range%'     => $this->renderTo($name, $value, $attributes, $errors),
        '%empty_checkbox%' => $this->getOption('with_empty') ? $this->renderTag('input', array('type' => 'checkbox', 'name' => $name.'[is_empty]', 'checked' => $values['is_empty'] ? 'checked' : '')) : '',
        '%empty_label%'    => $this->getOption('with_empty') ? $this->renderContentTag('label', $this->getOption('empty_label'), array('for' => $this->generateId($name.'[is_empty]'))) : '',
      ));            
    }
