...
[?php $module_filters = $sf_user->getAttribute('<?php echo $this->getModuleName() ?>.filters', null, 'admin_module') ?]
[?php if ($pager->count() || (!empty($module_filters) && $module_filters->count())): ?] 
    ...
    <tr>
      <td class="DataContainer" [?php if (!empty($filters)): ?]colspan="2"[?php endif ?]>
        <?php if ($this->configuration->getValue('list.batch_actions')): ?>
          <form action="[?php echo url_for('<?php echo $this->getUrlForAction('collection') ?>', array('action' => 'batch')) ?]" method="post" style="width: 100%">
        <?php endif; ?>
        [?php if ($pager->count()): ?]  
          [?php include_partial('<?php echo $this->getModuleName() ?>/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper, 'next_sort_type' => $next_sort_type, 'batch_ids' => !empty($batch_ids) ? $batch_ids : array())) ?]
        [?php else: ?]
        ...
        <ul class="BatchActions">
          [?php if ($pager->count()): ?]
            [?php include_partial('<?php echo $this->getModuleName() ?>/list_batch_actions', array('helper' => $helper, 'batch_action_id' => 'select_batch_action_2', 'batch_inactive_id' => 'select_batch_action_1')) ?]
          [?php endif ?]
        </ul>
  ...
  [?php include_partial('<?php echo $this->getModuleName() ?>/list_footer', array('helper' => $helper)) ?]

[?php else: ?]
  [?php include_partial('<?php echo $this->getModuleName() ?>/list_empty', array('helper' => $helper)) ?]
[?php endif ?]
