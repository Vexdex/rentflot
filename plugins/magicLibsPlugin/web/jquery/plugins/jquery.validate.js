	...
	rules: function(command, argument) {
		...
		if (command) {
			...
			case "add":
				...
				// changed from "name" to "id" ( 3 times)
...
$.extend($.validator, {
	...
	prototype: {
		...
		element: function( element ) {
			...
			// changed from "name" to "id" ( 2 times)
			if ( result ) {
      ...
      //alert(this.settings.showErrors);
    ...
    //alert(dump(this.settings));
      ...
      // changed from "name" to "id" ( 2 times)
		...
		findById: function( name ) {
			...
			return $(document.getElementsById(name)).map(function(index, element) {
	...
	// changed from "name" to "id" ( 1 time)
