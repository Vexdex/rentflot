...
function add_object_image(container_id)
{
  if ($('#' + container_id).length)
  {
      ...
      new_image_html += '<input name="object_images[' + photo_index + ']" id="object_images[' + photo_index + ']" type="file" />&nbsp;<img style="cursor: pointer" src="/magicLibsPlugin/images/delete.png" onclick="remove_image(\'new_image_' + photo_index + '\'); return false;" title="' + str_grid[CULTURE]['delete_image'] + '"/></td>';
      new_image_html += '</tr>';      
...
$(document).ready(function(){ 
  ...
  $("#object_images_sortable tbody:first").sortable({
    opacity: 0.6,
    cursor: 'move',
    cancel: '#add_object_image_control input',
    helper: 'clone',
    //helper: fixHelper,
    update: function() {
